...
const DateIdeaItem = React.memo(({ idea, isEditing, onToggleFinished, onDelete, onEdit, onSave, onCancel }) => {
    ...
    return (
        <div className={`date-idea-item ${idea.finished ? 'finished' : ''}`}>
            ...
            {!idea.finished && (
                <div className="item-actions">
                    <button className="button-icon edit-button" onClick={() => onEdit(idea.id)}><PencilIcon /></button>
                    <button className="button-icon delete-button" onClick={() => onDelete(idea)}><TrashIcon /></button>
                </div>
            )}
        </div>
    );
            ...
            <div className="page-container date-ideas-page">
                <div className="card date-ideas-card">
                    <div className="date-ideas-header">
                        ...
                        <button className="add-idea-button" onClick={() => setIsAddModalOpen(true)}><PlusIcon/></button>
                    </div>
                    <div className="date-ideas-list-container">
                        {loading ? ( <div className="centered-feedback"><SpinnerIcon/></div> ) : isListCompletelyEmpty ? (
                            ...
                            <>
                                {unFinishedIdeas.length > 0 && <div className="list-section"><h3>To Do<span className="count-badge">{unFinishedIdeas.length}</span></h3><div className="date-ideas-grid">{unFinishedIdeas.map(idea => ( <DateIdeaItem key={idea.id} idea={idea} isEditing={editingIdeaId === idea.id} onToggleFinished={toggleFinished} onDelete={handleDeleteRequest} onEdit={setEditingIdeaId} onSave={handleUpdateIdea} onCancel={() => setEditingIdeaId(null)} /> ))}</div></div>}
                                {finishedIdeas.length > 0 && <div className="list-section"><h3>Finished<span className="count-badge">{finishedIdeas.length}</span></h3><div className="date-ideas-grid">{finishedIdeas.map(idea => ( <DateIdeaItem key={idea.id} idea={idea} onToggleFinished={toggleFinished} onDelete={handleDeleteRequest} /> ))}</div></div>}
                            </>
                        )}
                    </div>
                </div>
            </div>
            <AddIdeaModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onAdd={handleAddIdea} isSubmitting={isSubmitting} />
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleConfirmDelete} title="Delete Date Idea?"> Are you sure you want to permanently delete <strong>{ideaToDelete?.title}</strong>? This action cannot be undone. </ConfirmationModal>
