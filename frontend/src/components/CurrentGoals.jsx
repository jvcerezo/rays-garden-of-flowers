...
const GoalItem = React.memo(
    ({ goal, onAddProgress, onToggleComplete, onEdit, onDelete, onCheckboxToggle }) => {
        const progress =
            goal.targetValue > 0 ? ((goal.currentValue || 0) / goal.targetValue) * 100 : 0;

        const renderCheckboxes = () => {
            if (!goal.targetValue || goal.targetValue > 50) {
                return <p className="checkbox-limit-message">({goal.targetValue} steps)</p>;
            }
            const checkboxes = [];
            for (let i = 1; i <= goal.targetValue; i++) {
                ...
                checkboxes.push(
                    <div
                        key={i}
                        className={`progress-checkbox ${isChecked ? 'checked' : ''}`}
                        onClick={() => onCheckboxToggle(goal, isChecked)}
                    >
...
const GoalModal = ({ isOpen, onClose, onSave, onDelete, isSubmitting, goal }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            const state = { ...initialFormState, ...(goal || {}) };
            setFormState(state);
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="modal-footer">
                        {goal && (
                            <button
                                type="button"
                                className="button delete-goal-button"
                                onClick={() => onDelete(goal)}
                            >
                                Delete Goal
                            </button>
                        ...
                        <button
                            type="submit"
                            className="button save-goal-button"
...
const AddProgressModal = ({ isOpen, onClose, onSave, isSubmitting, goal }) => {
    ...
    const handleChange = (e) => {
        ...
        if (goal && goal.targetValue) {
            ...
            if (value > remaining) {
                setError(`Amount cannot exceed the remaining ${formatValue(remaining)}.`);
            } else {
                setError('');
            }
        }
                    ...
                    <div className="modal-body">
                        <div className="form-group">
                            <input
                                ...
                                placeholder="Value to add (e.g., 500)"
                                value={amount}
...
function CurrentGoals() {
    const { goals, loading, addGoal, updateGoal, addProgress, toggleComplete, deleteGoal } =
        useGoals();
    ...
    const handleSaveProgress = async (amount) => {
        ...
        try {
            ...
            addToast(`${formatValue(amount)} ${selectedGoal.unit || ''} added!`);
            handleCloseProgressModal();
        } catch (err) {
            addToast('Failed to add progress.', 'error');
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleCheckboxToggle = async (goal, isChecked) => {
        ...
        if (
            goal.currentValue + amount > goal.targetValue ||
            goal.currentValue + amount < 0
        )
            return;
        try {
    ...
    return (
        <>
            ...
            >
                ...
                <strong>{selectedGoal?.title}</strong>? This action cannot be undone.
            </ConfirmationModal>
