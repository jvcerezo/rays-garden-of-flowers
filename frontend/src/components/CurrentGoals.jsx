...
const GoalModal = ({ isOpen, onClose, onSave, onDelete, isSubmitting, goal }) => {
    ...
    return (<div className="modal-backdrop" onClick={onClose}><div className="modal-content" onClick={e => e.stopPropagation()}><form onSubmit={handleSubmit}><div className="modal-header"><h2>{goal ? 'Edit Goal' : 'New Goal'}</h2><button type="button" className="close-modal-button" onClick={onClose}><CloseIcon /></button></div><div className="modal-body">{!isEditing && (<div className="goal-type-selector"><button type="button" className={type === 'financial' ? 'active' : ''} onClick={() => setType('financial')}>Financial</button><button type="button" className={type === 'general' ? 'active' : ''} onClick={() => setType('general')}>General</button></div>)}<input type="text" className="input-field-current-goals" placeholder={type === 'financial' ? "e.g., Save for a Trip" : "e.g., Read 20 Books"} value={formState.title} onChange={e => setFormState({...formState, title: e.target.value})} required autoFocus /><textarea className="input-field-current-goals" placeholder="Description (optional)" value={formState.description} onChange={e => setFormState({...formState, description: e.target.value})} rows="3"></textarea><div className="form-row"><input type="number" className="input-field-current-goals" placeholder="Target Value" value={formState.targetValue} onChange={e => setFormState({...formState, targetValue: Number(e.target.value)})} required />{type === 'general' && (<input type="text" className="input-field-current-goals" placeholder="Unit (e.g., books)" value={formState.unit} onChange={e => setFormState({...formState, unit: e.target.value})} />)}</div><input type="date" className="input-field" value={formState.deadline} onChange={e => setFormState({...formState, deadline: e.target.value})} /></div><div className="modal-footer">{goal && (<button type="button" className="button delete-goal-button" onClick={() => onDelete(goal)}>Delete Goal</button>)}<button type="submit" className="button save-goal-button" disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Save Goal'}</button></div></form></div></div>);
};
...
const AddProgressModal = ({ isOpen, onClose, onSave, isSubmitting, goal }) => {
    ...
    return (<div className="modal-backdrop" onClick={onClose}><div className="modal-content" onClick={e => e.stopPropagation()}><form onSubmit={handleSubmit}><div className="modal-header"><h2>Add Progress to "{goal.title}"</h2><button type="button" className="close-modal-button" onClick={onClose}><CloseIcon /></button></div><div className="modal-body"><div className="form-group"><input type="number" className={`input-field-current-goals ${error ? 'error' : ''}`} placeholder={`Value to add (e.g., 500)`} value={amount} onChange={handleChange} required autoFocus/>{error && <span className="validation-error">{error}</span>}</div></div><div className="modal-footer"><button type="button" className="button secondary-button" onClick={onClose}>Cancel</button><button type="submit" className="button save-goal-button" disabled={isSubmitting || !!error}>{isSubmitting ? 'Saving...' : 'Add'}</button></div></form></div></div>);
};
