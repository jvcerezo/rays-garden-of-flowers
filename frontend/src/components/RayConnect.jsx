...
function RayConnect() {
    ...
    useEffect(() => {
        ...
        const unsubRheana = onSnapshot(doc(db, 'rayConnect', PLAYER_IDS.rheanamindo), (docSnap) =>
            setPlayerData((prev) => ({ ...prev, rheanamindo: docSnap.data() }))
        );
        const unsubJet = onSnapshot(doc(db, 'rayConnect', PLAYER_IDS.jetjetcerezo), (docSnap) =>
            setPlayerData((prev) => ({ ...prev, jetjetcerezo: docSnap.data() }))
        );
        const qRheana = query(
            collection(db, 'rayConnect', PLAYER_IDS.rheanamindo, 'tasks'),
            orderBy('createdAt', 'desc')
        );
        const unsubTasksRheana = onSnapshot(qRheana, (snapshot) =>
            setTasks((prev) => ({
                ...prev,
                rheanamindo: snapshot.docs.map((d) => ({ id: d.id, ...d.data() })),
            }))
        );
        const qJet = query(
            collection(db, 'rayConnect', PLAYER_IDS.jetjetcerezo, 'tasks'),
            orderBy('createdAt', 'desc')
        );
        const unsubTasksJet = onSnapshot(qJet, (snapshot) => {
            setTasks((prev) => ({
                ...prev,
                jetjetcerezo: snapshot.docs.map((d) => ({ id: d.id, ...d.data() })),
            }));
            setIsLoading(false);
        });
        ...
        const promise = new Promise(async (resolve, reject) => {
            try {
                ...
                if (newExp >= nextLevelExp) {
                    ...
                    toast.success(`LEVEL UP! You are now Level ${playerUpdates.level}!`, {
                        duration: 4000,
                        icon: '🎉',
                    });
                } else {
