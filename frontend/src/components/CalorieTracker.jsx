...
function CalorieTracker() {
    ...
    useEffect(() => {
        ...
        const unsubscribeLibrary = onSnapshot(query(libraryRef), (snapshot) => {
            setFoodLibrary(snapshot.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() })));
        });
    ...
    const handleFetchHistory = async (date) => {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            const totalCalories = allMeals.reduce((sum, meal) => {
                return (
                    sum +
                    (docSnap.data()[meal] || []).reduce(
                        (mealSum, food) => mealSum + (food.calories || 0),
                        0
                    )
                );
            }, 0);
            monthHistory[docSnap.id] = totalCalories;
        });
    ...
    if (loading) {
        return <LoadingSpinner text="Loading Calorie Tracker..." />;
    }
            ...
            <div className="page-container calorie-tracker-page">
                <header className="tracker-header">
                    ...
                    <div className="header-title-container">
                        ...
                        <span className="header-subtitle">{format(new Date(), 'eeee, MMMM d')}</span>
                    </div>
                    <div className="header-right-actions">
                        <button
                            ...
                            title="Calorie History Calendar"
                            aria-label="Calorie Calendar"
                        >
                            <CalendarIcon />
                        </button>
                        <button
                ...
                >
                    ...
                    >
                        <div className="calendar-modal-top">
                            <h3 className="modal-title" style={{ margin: 0 }}>Calorie History</h3>
                            <button
                                onClick={() => setIsCalendarOpen(false)}
                        ...
                        <Calendar
                            onActiveStartDateChange={({ activeStartDate }) =>
                                handleFetchHistory(activeStartDate)
                            }
                            tileContent={({ date, view }) => {
                                if (view === 'month') {
                                    ...
                                    if (calories > 0) {
                                        return (
                                            <p className="calendar-day-calories">
                                                {calories}{' '}
