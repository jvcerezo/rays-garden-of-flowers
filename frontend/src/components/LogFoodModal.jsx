...
const NewFoodForm = ({ onSave, editingFood, mealType }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const foodData = { name, calories: parseInt(calories, 10) || 0 };
...
export function LogFoodModal({ onClose, onLogFood, foodLibrary, userId }) {
    ...
    const formattedMealType = selectedMeal
                ...
                {!selectedMeal ? (
                    <>
                        ...
                        <div className="meal-selection-grid">
                            {mealTypes.map((meal) => (
                                <button
                                    key={meal.key}
                                    className="meal-selection-button"
                    ...
                    <>
                        <div className="modal-header">
                            ...
                            <button
                                className={`header-tab ${view === 'create' ? 'active' : ''}`}
                                    ...
                                    {foodLibrary.length > 0 ? (
                                        foodLibrary.map((food) => (
                                            <div key={food.id} className="food-library-item">
                                                <div
                                                    className="food-item-log-area"
                                                    onClick={() => onLogFood(food, selectedMeal)}
                                                >
                                                    <span>{food.name}</span>
                                                ...
                                                <div className="food-item-actions">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() =>
                                                            handleDeleteFromLibrary(food.id, food.name)
                                                        }
                                                        title="Delete"
                                                    >
                                                        <TrashIcon />
                                        ...
                                        <p className="empty-library-message">
                                            Your food library is empty. Go to "Create New" to add a food.
                                        </p>
                                    )}
                                </div>
                            ) : (
                                <NewFoodForm
                                    onSave={handleSaveFoodToLibrary}
